    ...
    for (let zip of cvData.getTestZips()) {
        Scenario(`Test data for ${zip}`, (I) => {
            ...
            if (realDataItemsLen === 0) {
                ...
                const countyNames = realDataItems.map((i) => (i.county || {}).name).sort();
                ...
                if (zip && zip !== 'US') {
                    ...
                    log.debug(`Select random item in #${index} (length=${realDataItemsLen})`)
                    ...
                    log.debug(`Found real data county: ${realData ? realData.county.name : 'NULL'}`);
                } else {
